...
TesselateShader::TesselateShader()
{
	Shader();
	m_hullShader = 0;
	m_domainShader = 0;
...
TesselateShader::TesselateShader(const TesselateShader& other)
{
	Shader(static_cast<Shader>(other));
}

bool TesselateShader::Initialize(ID3D11Device* device, HWND hwnd)
{
	bool result;
	// Initialize the shaders.
	result = InitializeShader(device, hwnd, L"tesselate.vs", L"hull.hs", L"domain.ds", L"tesselate.ps");
	if (!result)
	{
		return false;
	}
...
bool TesselateShader::Render(ID3D11DeviceContext* deviceContext, int indexCount, XMMATRIX worldMatrix, XMMATRIX viewMatrix,
...
void TesselateShader::RenderShader(ID3D11DeviceContext* deviceContext, int indexCount)
{
...
bool TesselateShader::SetShaderParameters(ID3D11DeviceContext* deviceContext, XMMATRIX worldMatrix, XMMATRIX viewMatrix,
	XMMATRIX projectionMatrix, float tessellationAmount)
{
	HRESULT result;
...
bool TesselateShader::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* hsFilename, WCHAR* dsFilename, WCHAR* psFilename)
{
	HRESULT result;
...
void TesselateShader::ShutdownShader()
{
